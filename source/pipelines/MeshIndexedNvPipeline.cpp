...
MeshIndexedNvPipeline::MeshIndexedNvPipeline(SharedData* shared)
	:PipelineInterface(shared, "MeshIndexedNv")
{
}

void MeshIndexedNvPipeline::doInitialize(avk::queue* queue)
{
	for (size_t meshIndex = 0; meshIndex < mShared->mMeshData.size(); meshIndex++) {
		...
		cpuMeshlets = std::move(avk::divide_into_meshlets(meshletSelection, meshlet_division_meshoptimizer3, true, sNumVertices, sNumIndices - ((sNumIndices / 3) % 4) * 3));

		// We need to add vertex offset to the original indices because we only have one vertex array as oposed to one for each model
		auto [gpuMeshlets, gpuIndicesData] = avk::convert_for_gpu_usage<avk::meshlet_redirected_gpu_data, sNumVertices, sNumIndices> (cpuMeshlets, mShared->mMeshData[meshIndex].mVertexOffset);
	...
	mPackedIndexBuffer = avk::context().create_buffer(avk::memory_usage::device, {},
		avk::storage_buffer_meta::create_from_data(mPackedIndices)
	);
	mMeshletsBuffer = avk::context().create_buffer(avk::memory_usage::device, {},
	...
	avk::context().record_and_submit_with_fence({
		...
		mPackedIndexBuffer->fill(mPackedIndices.data(), 0)
	...
	auto meshletTaskShaderPath = ShaderMetaCompiler::precompile("meshlet.task", {
		{"MCC_MESHLET_EXTENSION", "_NV"}
	});

	mPipeline = avk::context().create_graphics_pipeline_for(
		avk::task_shader(meshletTaskShaderPath)
			.set_specialization_constant(0, mShared->mPropsMeshShaderNV.maxTaskWorkGroupInvocations),
		avk::mesh_shader("shaders/meshlet_indexed.nv.mesh")
		.set_specialization_constant(0, mShared->mPropsMeshShaderNV.maxTaskWorkGroupInvocations)
...
avk::command::action_type_command MeshIndexedNvPipeline::render(int64_t inFlightIndex)
{
	using namespace avk;
