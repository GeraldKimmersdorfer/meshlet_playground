...
void PDifferenceCodec8::doCompress(avk::queue* queue)
{
	...
	for (uint32_t mltid = 0; mltid < meshlets.size(); mltid++) {
		...
		bitCounts.position = glm::ceil(glm::vec3(bitCounts.position) / glm::vec3(8.0f)) * glm::vec3(8.0f);
		bitCounts.normal = glm::ceil(glm::vec2(bitCounts.normal) / glm::vec2(8.0f)) * glm::vec2(8.0f);
		bitCounts.texCoord = glm::ceil(glm::vec2(bitCounts.texCoord) / glm::vec2(8.0f)) * glm::vec2(8.0f);
		bitCounts.boneAttributes = (glm::ceil(glm::vec1(bitCounts.boneAttributes) / glm::vec1(8.0f)) * glm::vec1(8.0f)).x;
	...
	for (const auto& vdata : mTemporaryVertexData) {
		totalVertexBufferBytes += vdata.getFinalByteSize(1);
	}
	...
	// Fill the vertex and meshlet extension data aswell as set the vid to the byte offset
	{
		...
		for (int vid = 0; vid < mTemporaryVertexData.size(); vid++) {
			...
			vdata.addToDataVector(m8BitVertexData, 1);
		}
...
void PDifferenceCodec8::doDestroy()
{
	mBoneLUTData.clear();
...
void PDifferenceCodec8::hud_config(bool& config_has_changed)
{
