...
#include "OptimalSimplexCoding.h"
...
void OptimalSimplexCoding::doCompress(avk::queue* queue)
{
	...
	for (uint32_t vid = 0; vid < mShared->mVertexData.size(); vid++) {
		...
		newVert.tupleIndex = vertexLUIndexTable[vid];
		newVert.boneWeights = oss_compress(vert.mBoneWeights);
	}
...
void OptimalSimplexCoding::doDestroy()
{
	mVertexData.clear();
	mBoneLUTData.clear();
...
void OptimalSimplexCoding::hud_config(bool& config_has_changed)
{
