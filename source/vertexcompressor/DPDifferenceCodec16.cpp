...
void DPDifferenceCodec16::doCompress(avk::queue* queue)
{
	...
	for (uint32_t vid = 0; vid < mShared->mVertexData.size(); vid++) {
		...
		uint32_t payload = ((uint32_t(vertexLUPermutation[vid]) & 0x1F) << 13) | (uint32_t(vertexLUIndexTable[vid]) & 0x1FFF);
		newVert.boneAttributes = PermutationCodec::encode(vert.mBoneWeights, payload, PermutationCodec::codec14bit);
	}
	...
	for (uint32_t mltid = 0; mltid < meshlets.size(); mltid++) {
		...
		bitCounts.position = glm::ceil(glm::vec3(bitCounts.position) / glm::vec3(16.0f)) * glm::vec3(16.0f);
		bitCounts.normal = glm::ceil(glm::vec2(bitCounts.normal) / glm::vec2(16.0f)) * glm::vec2(16.0f);
		bitCounts.texCoord = glm::ceil(glm::vec2(bitCounts.texCoord) / glm::vec2(16.0f)) * glm::vec2(16.0f);
		bitCounts.boneAttributes = (glm::ceil(glm::vec1(bitCounts.boneAttributes) / glm::vec1(16.0f)) * glm::vec1(16.0f)).x;
	...
	std::vector<meshlet_native> newMeshlets;
	{
		...
		for (auto& pair : mVertexReferences) {
			if (pair.second.size() > 1) {
				...
				const auto& firstMinValues = minimumVertexDataForMeshlets[pair.second[0]];
				const auto& firstBitCounts = bitCountsForMeshlets[pair.second[0]];
				for (int i = 1; i < pair.second.size(); i++) {
					...
					if (refMinValues.boneAttributes != firstMinValues.boneAttributes ||
						refBitCounts.boneAttributes != firstBitCounts.boneAttributes) {
						compatible = false;
						break;
					}
				}
	...
	// Fill the vertex and meshlet extension data aswell as set the vid to the byte offset
	{
		...
		for (int mid = 0; mid < newMeshlets.size(); mid++)
		{
			...
			meshletExtension.boneAttributesMin = minimums.boneAttributes;
			meshletExtension.boneAttributesResolution = bitCounts.boneAttributes / 8;
		}
	}
...
void DPDifferenceCodec16::doDestroy()
{
	mBoneLUTData.clear();
...
void DPDifferenceCodec16::hud_config(bool& config_has_changed)
{
