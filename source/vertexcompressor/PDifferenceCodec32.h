...
class PDifferenceCodec32 : public VertexCompressionInterface {
	...
	struct cpu_compressed_vertex_data {
		...
		glm::u16vec4 originalBoneAttributes;
	}; // 20 bytes
	...
	struct cpu_temporary_vertex_data {
		...
		uint8_t boneAttributesResolution; // 0, 4
		glm::u16vec4 originalBoneAttributes;
	...
	struct meshlet_extension {
		...
		uint16_t w3;
		uint16_t w4;
	}; // has to be 8 byte aligned

public:

	PDifferenceCodec32(SharedData* shared)
		: VertexCompressionInterface(shared, "P. Difference Codec 32bit", "_PDC32")
	{}
